        ...
        || level.vesd ) )
    {
      while( ent->nextRegenTime < level.time )
      {
        ...
        else
        {
          ...
          ent->nextRegenTime = level.time + floor( VAMPIRIC_ESD_TTD/client->ps.stats[ STAT_MAX_HEALTH ] );
          continue;
        }
    ...
    && ent->health > 0 && ( ent->nextRegenTime < level.time || level.vesd ) )
  {
    ...
    if( ent->nextRegenTime < level.time && regen > 0 )
    {
      ...
      else
      {
        //ent->health -= 1;
        G_Damage( ent, NULL, NULL, NULL, NULL, 1, 0, MOD_VESD );
        ent->nextRegenTime = level.time + ceil( VAMPIRIC_ESD_TTD/client->ps.stats[ STAT_MAX_HEALTH ] );
      }
      //take away one tk credit
      for( i = 0; i < MAX_CLIENTS; i++ )
      {
