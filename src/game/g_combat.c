...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
{
  ...
  float     totalDamage = 0.0f, totalTK = 0.0f;
  qboolean  tk = qfalse;
  ...
  if( attacker && attacker->client )
  {
    ...
    if( g_retribution.integer && totalTK && !g_tkmap.integer )
    {
      ...
      {
        totalPrice = BG_ClassCanEvolveFromTo( PCL_ALIEN_LEVEL0, self->client->ps.stats[ STAT_CLASS ], self->client->ps.persistant[ PERS_CREDIT ], g_alienStage.integer, 0, 0 );
...
//   if( totalDamage > 0.0f && 1==2)//FIXME: WIRE: I disabled this section because it is broken as it need to be updated for the new balance.
//   {
//     if( self->client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
//     {
         ...
         vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
  ...
  if( targ->s.eType == ET_MOVER )
  {
    ...
    if( attacker->client->pers.teamSelection == TEAM_ALIENS 
        && !( mod == MOD_LEVEL2_ZAP || mod == MOD_POISON || mod == MOD_INFECTION ) )
    {
