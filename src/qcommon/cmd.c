...
void Cmd_If_f( void ) {
  ...
  if ( (Cmd_Argc () == 6 ) || (Cmd_Argc () == 5) ) {
    v1 = Cvar_VariableValue( Cmd_Argv( 1 ) );
    op = Cmd_Argv( 2 );
    v2 = Cvar_VariableValue( Cmd_Argv( 3 ) );
    ...
    Com_Printf ("if <variable1> <operator> <variable2> <cmdthen> (<cmdelse>) : compares the first two values and executes <cmdthen> if true, <cmdelse> if false\n");
    return;
  }
  Cbuf_InsertText( va("%s\n", v ) );
}
...
void Cmd_Math_f( void ) {
  ...
  else if (Cmd_Argc () == 4)
  {
    ...
    if ( !strcmp( op, "+" ) )
    {
      ...
      Cvar_SetValueLatched( v, ( Cvar_VariableValue( v ) + Cvar_VariableValue( v1 ) ) );
    }
    else if ( !strcmp( op, "-" ) )
    {
      Cvar_SetValueLatched( v, ( Cvar_VariableValue( v ) - Cvar_VariableValue( v1 ) ) );
  ...
  else if (Cmd_Argc () == 6)
  {
    ...
    if ( !strcmp( op, "+" ) )
    {
      Cvar_SetValueLatched( v, ( Cvar_VariableValue( v1 ) + Cvar_VariableValue( v2 ) ) );
    }
    else if ( !strcmp( op, "-" ) )
    {
      Cvar_SetValueLatched( v, ( Cvar_VariableValue( v1 ) - Cvar_VariableValue( v2 ) ) );
