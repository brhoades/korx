...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
{
  ...
  self->client->ps.persistant[ PERS_KILLED ]++;
   ...
   if( attacker && attacker->client && !tk && ( attacker->client->pers.netname != self->client->pers.netname ) && 
		!( self->client->infected && self->client->infector->client->pers.netname == self->enemy->client->pers.netname ) )
   {
  ...
  if( attacker && attacker->client )
  {
    ...
    {
      if( OnSameTeam( self, attacker ) && !(self->client->infected) )
      {
        AddScore( attacker, -1 );
...

  {
    ...
    if (!(i<0 || i>2)) //seems we get a negative one every now and then, so we need to correct for this
    {
