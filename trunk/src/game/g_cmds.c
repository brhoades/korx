...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText )
{
  ...
  qboolean    actionTeam = qfalse;
 
  if( g_actionPrefix.string[0] && ( !Q_stricmpn( chatText, "/me ", 4 ) ||
      ( actionTeam = !Q_stricmpn( chatText, "/me_team ", 9 ) ) ) )
  {
    qboolean team = ( mode == SAY_TEAM || actionTeam );
  ...
  if( g_floodMinTime.integer )
    ...
    G_LogPrintf( "action%s: %s" S_COLOR_WHITE ": %s\n", ( team ) ? "team" : "",
        ent->client->pers.netname, chatText );

    Com_sprintf( name, sizeof( name ), "%s%s" S_COLOR_WHITE "%s " 
      S_COLOR_WHITE, ( team ) ? S_COLOR_CYAN : S_COLOR_GREEN, 
      g_actionPrefix.string, ent->client->pers.netname );
  }
  else
  { // I'm going to forgo proper indentation here in favour of a smaller diff
...
static void Cmd_Say_f( gentity_t *ent )
{
  ...
  if( Q_stricmpn( args, "say_admins ", 11 ) == 0 || Q_stricmpn( args, "a ", 2 ) == 0)
    mode = SAY_ADMINS;
  if( Q_stricmpn( args, "say_clan ", 10 ) == 0 || Q_stricmpn( args, "c ", 2 ) == 0)
    mode = SAY_CLAN;
  ...
  if( !Q_stricmpn( args, "say /c ", 7) ||
      ...
      !Q_stricmpn( args, "say /say_clan ", 15) ||
...
//       return;
		...
		else
		{
			...
			Com_sprintf( level.voteString, sizeof( level.voteString ),
				"!ban %s %d vote kick: %s", level.clients[ clientNum ].pers.ip,
        ...
        else if( !Q_stricmp( arg1, "denybuild" ) )
  {
    ...
    {
      trap_SendServerCommand( ent-g_entities,
        "print \"callvote: admin is immune from denybuild\n\"" );
      return;
    }

    Com_sprintf( level.voteString,
      sizeof( level.voteString ), "!denybuild %i", clientNum );
  ...
  else if( !Q_stricmp( arg1, "allowbuild" ) )
  {
    ...
    Com_sprintf( level.voteString,
      sizeof( level.voteString ), "!allowbuild %i", clientNum );
		if( !g_extravotereasons.value )
   	  Com_sprintf( level.voteDisplayString,
          sizeof( level.voteDisplayString ),
  ...
  else if( !Q_stricmp( arg1, "map_restart" ) )
  {
		...
		{
			trap_SendServerCommand( ent - g_entities,
				"print \"callvote: restart votes not allowed during extreme sudden death\n\"" );
			return;
		}
		if( level.suddenDeath )
		{
			trap_SendServerCommand( ent - g_entities,
				"print \"callvote: restart votes not allowed during sudden death\n\"" );
			return;
		}
	...
	else if( !Q_stricmp( arg1, "map" ) )
	{
		...
		{
			trap_SendServerCommand( ent - g_entities,
				"print \"callvote: map votes not allowed during extreme sudden death\n\"" );
			return;
		}
		if( level.suddenDeath )
		{
			trap_SendServerCommand( ent - g_entities,
				"print \"callvote: map votes not allowed during sudden death\n\"" );
			return;
		}
  ...
  else if( !Q_stricmp( arg1, "sudden_death" ) )
  {
    ...
    else if(!g_suddenDeathVotePercent.integer)
    {
  ...
  else if( !Q_stricmp( arg1, "extreme_sudden_death" ) )
  {
    if( level.extremeSuddenDeath )
    {
      ...
      return;
		...
		else if(level.time-level.startTime < (g_extremesuddenDeathVoteMinTime.integer*60000) )
		{
      trap_SendServerCommand( ent - g_entities,
        "print \"callvote: it is too early in the game to call esd\n\"" );
      return;
		}
     else if(!g_extremesuddenDeathVotePercent.integer)
     {
       trap_SendServerCommand( ent-g_entities, "print \"Extreme Sudden Death votes have been disabled\n\"" );
       return;
     }
    else if( level.extremeSuddenDeathWarning == ( TW_CLOSE || TW_IMMINENT ) )
    {
      trap_SendServerCommand( ent - g_entities,
        "print \"callvote: it is too close to extreme sudden death\n\"" );
      return;
    }
    level.votePercentToPass = g_extremesuddenDeathVotePercent.integer;

		if( !g_extremeSuddenDeathVote.integer )
		{
   	  Com_sprintf( level.voteString, sizeof( level.voteString ),
    	  "set g_extremeSuddenDeathVote 1" );
    	Com_sprintf( level.voteDisplayString, sizeof( level.voteDisplayString ),
      	"Begin Extreme Sudden Death" );
		}
		else
		{
			trap_SendServerCommand( ent-g_entities, "print \"An Extreme Sudden Death vote has already been called & passed.\n\"" );
			return;
		}
...
void Cmd_CallTeamVote_f( gentity_t *ent )
{
  ...
  else
  {
    ...
    trap_SendServerCommand( ent-g_entities,
       ...
       "kick, denybuild, allowbuild, poll, custom, designate, undesignate, and admitdefeat\n\"" );
    return;
  }
  ent->client->pers.voteCount++;
...
#define AS_OVER_RT3         ((ALIENSENSE_RANGE*0.20f)/M_ROOT3)
...
void Cmd_Destroy_f( gentity_t *ent )
{
  ...
  if( ent->client->ps.stats[ STAT_STATE ] & SS_HOVELING )
    if( ( ent->client->hovel->s.eFlags & EF_DBUILDER ) &&
      !ent->client->pers.designatedBuilder )
    {
			//If they're spamming, don't print this message
 			if( g_floodMinTime.integer )
  		  if ( G_Flood_Limited( ent ) )
    		 {
  	     trap_SendServerCommand( ent-g_entities, "print \"You're spamming! You don't have the ability to deconstruct protected structures, so please quit. Your chat has been disabled temporarily.\n\"" );
   		   return;
		    }
...
void Cmd_Buy_f( gentity_t *ent )
{
  ...
  if( weapon != WP_NONE )
  {
    ...
    {
      qboolean isEnergy=BG_FindUsesEnergyForWeapon( weapon );
      //battle suit has both battpack and ammopack - integrated :P
      if( isEnergy &&
...
commands_t cmds[ ] = {
  ...
  { "say_clan", CMD_MESSAGE|CMD_INTERMISSION, Cmd_Say_f },
  { "a", CMD_MESSAGE|CMD_INTERMISSION, Cmd_Say_f },
  { "c", CMD_MESSAGE|CMD_INTERMISSION, Cmd_Say_f },
