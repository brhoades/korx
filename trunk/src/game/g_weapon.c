...
void G_GiveClientMaxAmmo( gentity_t *ent, qboolean buyingEnergyAmmo )
{
  ...
  for( i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++ )
  {
          ...
          ent->client->ps.ammo, ent->client->ps.powerups ) )
    {
      ...
      {
        if( BG_InventoryContainsUpgrade( UP_AMMOPACK, ent->client->ps.stats ) ||
            BG_InventoryContainsUpgrade( UP_BATTLESUIT, ent->client->ps.stats ) )
...
void XChargeFire( gentity_t *ent, qboolean secondary )
{
  ...
  if( secondary )
  {
    m = fire_xael( ent, muzzle, forward, XAEL_SECONDARY_DAMAGE,
      XAEL_SECONDARY_RADIUS );
    
    ent->client->ps.weaponTime = XAEL_REPEAT;
  }
  else
  {
    m = fire_xael( ent, muzzle, forward, ent->client->ps.stats[ STAT_MISC ], XAEL_RADIUS );
    ent->client->ps.weaponTime = XAEL_CHARGEREPEAT;
  }
...
void cancelBuildFire( gentity_t *ent )
{
  ...
  if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS )
  {
    ...
    meleeAttack( ent, ABUILDER_CLAW_RANGE, ABUILDER_CLAW_WIDTH,
                 ABUILDER_CLAW_DMG/2, MOD_ABUILDER_CLAW ); //melee attack for alien builder
}
...
qboolean CheckVenomAttack2( gentity_t *ent )
{
  ...
  //allow bites to work against (human) buildables only
  if( traceEnt->s.eType == ET_BUILDABLE && traceEnt->health > 0 )
  {
...
void CheckGrabAttack( gentity_t *ent )
{
  ...
  if( traceEnt->client )
  {
    ...
    if( !( traceEnt->client->ps.stats[ STAT_STATE ] & SS_GRABBED ) )
    {
      ...
      if( BG_UpgradeIsActive( UP_JETPACK, traceEnt->client->ps.stats) && traceEnt->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS )
...
void poisonCloud( gentity_t *ent )
{
  ...
  for( i = 0; i < num; i++ )
  {
    ...
    {
			//WIRE: changed this to helmet so it would do something in tremx
...
void FireWeapon2( gentity_t *ent )
{
  if( ent->client )
  {
    if( ent->client->pers.paused ) return; 
    // set aiming directions
    AngleVectors( ent->client->ps.viewangles, forward, right, up );
    CalcMuzzlePoint( ent, forward, right, up, muzzle );
  }
