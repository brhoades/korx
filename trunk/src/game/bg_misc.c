...
classAttributes_t bg_classList[ ] =
{
  ...
  },
  {
    ...
    "Soldier Upgrade",                              //char    *humanname;
    "jumper",                                       //char    *modelname;
    0.2f,                                           //float   modelScale;
    "default",                                      //char    *skinname;
    0.3f,                                           //float   shadowScale;
  ...
  },
  {
    ...
    1.0f,                                           //float   shadowScale;
    "alien_general_hud",                            //char    *hudname;
    ( 1 << S2 )|( 1 << S3 ),                      //int  stages;                                    //int  stages
    { -32, -32, -21 },                              //vec3_t  mins;
    { 32, 32, 21 },                                 //vec3_t  maxs;
    { 32, 32, 21 },                                 //vec3_t  crouchmaxs;
    { -32, -32, -4 },                               //vec3_t  deadmins;
  ...
  },
  {
    ...
    "Big Mofo Upgrade",                             //char    *humanname;
    "mofo",                                         //char    *modelname;
    1.0f,                                           //float   modelScale;
    "default",                                      //char    *skinname;
    2.0f,                                           //float   shadowScale;
...
void BG_EvaluateTrajectory( const trajectory_t *tr, int atTime, vec3_t result )
{
  ...
  switch( tr->trType )
  {
    ...
    //thanks to corncobman
    case TR_SPIRAL:
      deltaTime = ( atTime - tr->trTime ) * 0.001;   // milliseconds to seconds

      if(tr->trDelta[1] == 0.0f && tr->trDelta[2] == 0.0f)
         VectorSet(B, 0.0f, 1.0f, 0.0f);
      else VectorSet(B, 1.0f, 0.0f, 0.0f);

      CrossProduct (tr->trDelta, B, A);
      CrossProduct (tr->trDelta, A, B);
      VectorNormalize(A);
      VectorNormalize(B);

      VectorScale (A, sin( 5*deltaTime + tr->trDuration) * 15, A);
      VectorScale (B, cos( 5*deltaTime + tr->trDuration) * 15, B);
      VectorMA( tr->trBase, deltaTime, tr->trDelta, result );
      VectorAdd( result, B, result);
      VectorAdd( result, A, result);
      break;
...
qboolean BG_WeaponIsFull( weapon_t weapon, int stats[ ], int psAmmo[ ], int psAmmo2[ ] )
{
  ...
  if( ( BG_InventoryContainsUpgrade( UP_BATTPACK, stats ) ||
      BG_InventoryContainsUpgrade( UP_BATTLESUIT, stats ) ) && isEnergy )
