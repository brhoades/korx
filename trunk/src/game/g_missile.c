...
void G_MissileImpact( gentity_t *ent, trace_t *trace )
{
  ...
  else if( !strcmp( ent->classname, "slowblob" ) )
  {
    if( other->client )
    {
      ...
      else if( other->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS )
      {
  ...
  else if( !strcmp( ent->classname, "stsblob" ) )
  {
    if( other->client )
    {
      if( ( ( other->client->ps.stats[ STAT_STATE ] & SS_SLOWLOCKED ) || ( other->client->ps.stats[ STAT_STATE ] & SS_SLOWLOCKED2 ) )  &&
      ...
      {
        other->client->ps.stats[ STAT_STATE ] |= SS_SLOWLOCKED2;
        other->client->lastSlowTime = level.time;
        other->client->blobs++;
      }
      else if( other->s.weapon < WP_ALEVEL3 )
      {
        other->client->ps.stats[ STAT_STATE ] |= SS_SLOWLOCKED;
        other->client->lastSlowTime = level.time;
      }
...
=================
*/
gentity_t *fire_xael( gentity_t *self, vec3_t start, vec3_t dir, int damage, int radius )
{
  gentity_t *bolt;
  int localDamage = (int)( ceil( ( (float)damage /
                                   (float)XAEL_TOTAL_CHARGE ) * (float)XAEL_DAMAGE ) );
  ...
  bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;   // move a bit on the very first frame
  VectorCopy( start, bolt->s.pos.trBase );
  VectorScale( dir, XAEL_SPEED, bolt->s.pos.trDelta );
  SnapVector( bolt->s.pos.trDelta );      // save net bandwidth
...
gentity_t *fire_eBlob( gentity_t *self, vec3_t start, vec3_t dir )
{
  ...
  bolt->splashMethodOfDeath = MOD_LEVEL4_EBLOB;
  bolt->clipmask = MASK_SHOT;
  bolt->target_ent = NULL;
...
gentity_t *fire_bounceBall( gentity_t *self, vec3_t start, vec3_t dir,
              int weapon, int dmg, int mod, int speed)
{
  gentity_t *bolt;
