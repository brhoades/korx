...
void G_CalculateBuildPoints( void )
{
  ...
  int         localHTP = g_humanBuildPoints.integer,
	...
	{
		if( (g_suddenDeath.integer || G_TimeTilSuddenDeath( ) <= 0 ) && !g_extremeSuddenDeath.integer) //Conditions to enter SD
		{
			//begin sudden death
...
// I am not sure why we were checking to see if level.warmupTime is 0
//	else if( g_extremeSuddenDeathTime.integer && !level.warmupTime )
	else if( g_extremeSuddenDeathTime.integer )
	{
		//warn about extreme sudden death
