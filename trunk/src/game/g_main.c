...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  { &g_extremeSuddenDeathVote, "g_extremeSuddenDeathVote", "0", CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, 0, qfalse },
  ...
  { &g_doWebsiteSpam, "g_doWebsiteSpam", "0", CVAR_ARCHIVE, 0, qtrue  },
  ...
  { &g_devmapKillerHP, "g_devmapKillerHP", "0", CVAR_ARCHIVE, 0, qfalse  },
...
void G_DoSpamMessages( void )
{
	...
	{
		if ( level.time - level.startTime > g_warmup.integer*1000 - 10000 && level.spamWebsite < TW_IMMINENT && g_doWebsiteSpam.integer)
		{
		...
		else if ( level.time - level.startTime > g_warmup.integer*1000 - 7500 && level.spamWebsite < TW_PASSED && g_doWebsiteSpam.integer)
		{
...
void G_CalculateBuildPoints( void )
{
  ...
  int         localHTP = g_humanBuildPoints.integer,
	...
	if( !level.suddenDeath && !level.extremeSuddenDeath )
	{
		...
		else if ( g_suddenDeathTime.integer )
		{
	...
	{
		if ( g_extremeSuddenDeath.integer || G_TimeTilExtremeSuddenDeath( ) <= 0  ) // conditions to enter extreme sudden death
		{
		...
		else if( g_extremeSuddenDeathTime.integer ) // esd warnings if we are using it time based
		{
			//warn about extreme sudden death when 5 minutes are left
			if( G_TimeTilExtremeSuddenDeath( ) <= 300000 && level.extremeSuddenDeathWarning < TW_CLOSE )
			{
				trap_SendServerCommand( -1, "cp \"Extreme Sudden Death in 5 minutes!\"" );
...
void ExitLevel( void )
{
  ...
  if ( !g_scrimMode.integer )
  {
	...
	{
		trap_SendConsoleCommand( EXEC_APPEND, "!restart switchteams\n" );
	}
}
...
void CheckVote( void )
{
  ...
  if( level.time - level.voteTime >= VOTE_TIME || ( level.voteYes + level.voteNo == level.numConnectedClients ) )
  {
    ...
    else if( level.voteYes == 0 && level.voteNo != 0 )
    {
  ...
  {
    if( participantpercent >= g_mapvotepart.integer && voteYesPercent > votePercentToPass && mapvote && level.voteYes > (int)((double)level.numConnectedClients * ((double)votePercentToPass/100.0)) && !mapvote )
    {
      // execute the command, then remove the vote
...
void G_RunFrame( int levelTime )
{
  ...
  //Stop the game time if we are paused
  ...
  for( i = 0; i < level.num_entities; i++, ent++ )
  {
    ...
    {
     if( gamepaused )				//Dont worry about thinking if we're paused.
      return;
      G_RunMissile( ent );
      continue;
    }
