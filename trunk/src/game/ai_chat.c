...
int BotIsFirstInRankings(bot_state_t *bs) {
  ...
  score = bs->cur_ps.persistant[PERS_KILLS];
  for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
    trap_GetConfigstring(CS_PLAYERS+i, buf, sizeof(buf));
    //if no config string or no name
    if (!strlen(buf) || !strlen(Info_ValueForKey(buf, "n"))) continue;
    ...
    if (score < ps.persistant[PERS_KILLS]) return qfalse;
  }
  return qtrue;
}
...
char *BotFirstClientInRankings(void) {
  ...
  for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
    ...
    //
    BotAI_GetClientState(i, &ps);
    if (ps.persistant[PERS_KILLS] > bestscore) {
      bestscore = ps.persistant[PERS_KILLS];
      bestclient = i;
    }
  }
...
char *BotLastClientInRankings(void) {
  ...
  for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
    ...
    //
    BotAI_GetClientState(i, &ps);
    if (ps.persistant[PERS_KILLS] < worstscore) {
      worstscore = ps.persistant[PERS_KILLS];
      bestclient = i;
    }
  }
  EasyClientName(bestclient, name, 32);
