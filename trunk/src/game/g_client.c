...
/*
===========
ClientCheckNameAdmin
============
*/
static void ClientCleanNameAdmin( const char *in, char *out, int outSize )
{
  int   len, colorlessLen;
  char  ch;
  char  *p;
...
void ClientUserinfoChanged( int clientNum )
{
  ...
  s = Info_ValueForKey( userinfo, "name" );
        if ( ent->client->pers.adminLevel <= 0 )
        {
                ClientCleanName( s, newname, sizeof( newname ) );
        }
        else
        {
                ClientCleanNameAdmin( s, newname, sizeof( newname ) );
        }
