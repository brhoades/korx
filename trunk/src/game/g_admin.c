...
qboolean G_admin_kick( gentity_t *ent, int skiparg )
{
  ...
  if( g_adminTempBan.integer > 0 )
  {
    admin_create_ban( ent,
      ...
      va("kick: %s", ( *reason ) ? reason : "kicked by admin") );
    if( g_admin.string[ 0 ] )
      admin_writeconfig();
  }
...
/* functions for commands added by wireddd */
qboolean G_admin_putmespec( gentity_t *ent, int skiparg )
{
  ...
  //Start the complicated nasty stuff to keep people from abusing this command.
	if( (( currentClass == PCL_ALIEN_BUILDER0 || currentClass == PCL_ALIEN_BUILDER0_UPG )
			|| ( weapon == WP_HBUILD || weapon == WP_HBUILD2 ))  &&
  	ent->client->ps.stats[ STAT_MISC ] > 0 )
  {
		trap_SendServerCommand( ent-g_entities,
  		va( "print \"You cannot move yourself to spectators until your build timer expires.\n\"" ) );
		return qfalse;
	}
...
qboolean G_admin_register(gentity_t *ent, int skiparg ){
  ...
  {
    ADMP( va( "^3!register: ^7 You cannot register for name protection until you update your client. Please replace your client executable with the one from the downloads page on knightsofreason.org and reconnect. Updating your client will also allow you to have faster map downloads.\n" ) );
    return qfalse;
  }
...
#define LOG_DISPLAY_LENGTH 10
    ...
    for( i = 1; i + skiparg < G_SayArgc( ); i++ )
    {
      ...
      switch( argbuf[ 0 ])
      {
        ...
        case '-':
          {
            //int value;
            //if( ( value = G_ClientNumbersFromString( argbuf + 1, pids ) ) != 1 )
  ...
  *message = '\0';
  ...
  if( matchlen )
    ADMP( va( "%s^3!buildlog: showing log entries %d - %d of %d available\n", message,
