...
static float PM_CmdScale( usercmd_t *cmd )
{
  ...
  if( ( pm->ps->weapon == WP_ALEVEL4 || pm->ps->weapon == WP_ALEVEL4_UPG ) && pm->ps->pm_flags & PMF_CHARGE )
...
static void PM_Weapon( void )
{
  ...
  switch( pm->ps->weapon )
  {
    ...
    case WP_XAEL:
      ...
      if( ( attack2 || pm->ps->stats[ STAT_MISC ] == 0 ) && !attack1 && !attack3 )
      {
        if( pm->ps->stats[ STAT_MISC ] < XAEL_TOTAL_CHARGE )
        {
          pm->ps->weaponTime = 0;
          pm->ps->weaponstate = WEAPON_READY;
          return;
      ...
      //erp this looks confusing
      if( pm->ps->stats[ STAT_MISC ] > XAEL_MIN_CHARGE )
        attack2 = !attack2;
      else if( pm->ps->stats[ STAT_MISC ] > 0 )
      {
        pm->ps->stats[ STAT_MISC ] = 0;
  ...
  if( attack3 )
  {
    if( BG_WeaponHasThirdMode( pm->ps->weapon ) )
    {
      ...
      else if( pm->ps->weapon == WP_ALEVEL2_UPG && !ammo )
      {
        PM_AddEvent( EV_NOAMMO );
        pm->ps->weaponTime += 200;
        return;
      }
      else if( pm->ps->weapon == WP_ALEVEL4_UPG && !ammo )
      {
        PM_AddEvent( EV_NOAMMO );
        pm->ps->weaponTime += 200;
        return;
  ...
  if( !BG_FindInfinteAmmoForWeapon( pm->ps->weapon ) )
  {
    ...
    else if( pm->ps->weapon == WP_XAEL && attack2 && !attack1 )
    {
      ammo -= (int)( ceil( ( (float)pm->ps->stats[ STAT_MISC ] / (float)XAEL_TOTAL_CHARGE ) * 10.0f ) );

      //stay on the safe side
      if( ammo < 0 )
        ammo = 0;
    }
    //special case for shotgunnade
    else if( pm->ps->weapon == WP_SHOTGUN && attack2 )
    {
      ammo -= SHOTGUN_NADE_TAKE;
  ...
  else if( pm->ps->weapon == WP_ALEVEL2_UPG && attack3 )
  {
    //special case for slowblob
    ammo--;
    BG_PackAmmoArray( pm->ps->weapon, pm->ps->ammo, pm->ps->powerups, ammo, clips );
  }
  else if( pm->ps->weapon == WP_ALEVEL4_UPG && attack3 )
  {
    //special case for eblob
    ammo--;
    BG_PackAmmoArray( pm->ps->weapon, pm->ps->ammo, pm->ps->powerups, ammo, clips );
  }
