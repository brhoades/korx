    ...
    if( self->s.groundEntityNum )
    {
              ...
              self->s.origin2, BA_A_SPAWN, NULL ) ) != NULL )
      {
            ...
            ent->s.eType == ET_MOVER )
        {
		 ...
		 ent->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS && !ent->client->pers.paused )
	{
...
void AOvermind_Think( gentity_t *self )
{
  ...
  if( self->spawned && ( self->health > 0 ) )
  {
	...
	continue;
      ...
      if( enemy->client && enemy->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS && !enemy->client->pers.paused )
      {
        self->timestamp = level.time;
        G_SelectiveRadiusDamage( self->s.pos.trBase, self, self->splashDamage,
...
void ABooster_Touch( gentity_t *self, gentity_t *other, trace_t *trace )
{
  ...
  if( ( client->lastBoostedTime + 5 ) > level.time )
    return;
  ...
  if( self->spawned )
  {
    ...
    if( self->powered )
      	...
      	G_TriggerMenu( activator->client->ps.clientNum, MN_H_ARMOURY );
			//If we're accessing the armoury, reset anything someone owes us, to prevent people exploiting this for free credits.
   // 	for( i = 0; i < MAX_CLIENTS; i++ )			---Doesn't work
    	//	activator->enemy->client->tkcredits[ i ] = 0;
			}
    else
...
void HMedistat_Think( gentity_t *self )
{
  ...
  if( self->spawned )
  {
    ...
    else if( self->enemy && !self->enemy->client->pers.paused) //heal!
    {
...
void HForceField_Use( gentity_t *self, gentity_t *other, gentity_t *activator )
{
  ...
  if( !client || client->pers.teamSelection != PTE_HUMANS )
    return;
...
void HSpawn_Die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
  ...
  if( self->s.modelindex == BA_H_FORCEFIELD )
  	HForceField_Shrink( self, qtrue );
    ...
    if( self->s.groundEntityNum )
    {
              ...
              self->s.origin2, BA_H_SPAWN, NULL ) ) != NULL )
      {
            ...
            ent->s.eType == ET_MOVER )
        {
		 ...
		 ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS && !ent->client->pers.paused )
	{
	  //spawnblock protection
