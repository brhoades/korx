...
void ClientTimerActions( gentity_t *ent, int msec )
{
  ...
  if( client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS &&
    	level.surrenderTeam != PTE_HUMANS && ent->health > 0 && 
    	ent->health < client->ps.stats[ STAT_MAX_HEALTH ] &&
      ( ent->lastDamageTime + ALIEN_REGEN_DAMAGE_TIME ) < level.time && 
      ent->nextHealTime < level.time )
  {
  ...
  while ( client->time100 >= 100 )
  {
    ...
    if( client->ps.weapon == WP_ALEVEL0_UPG )
    {
      if( client->ps.stats[ STAT_MISC ] < LEVEL0_DRILL_SPEED && ucmd->buttons & BUTTON_ATTACK2 )
        client->ps.stats[ STAT_MISC ] += ( 100.0f / (float)LEVEL0_DRILL_CHARGE_TIME ) * LEVEL0_DRILL_SPEED;
      ...
      if( client->ps.stats[ STAT_MISC ] > LEVEL0_DRILL_SPEED )
        client->ps.stats[ STAT_MISC ] = LEVEL0_DRILL_SPEED;
    }

    //client is charging up for a... charge
    if( client->ps.weapon == WP_ALEVEL4 || client->ps.weapon == WP_ALEVEL4_UPG )
    {
    ...
    //client is charging up a xael
    if( client->ps.weapon == WP_XAEL )
    {
      int ammo;
     ...
     if( client->ps.stats[ STAT_MISC ] > ( LCANNON_TOTAL_CHARGE - ( XAEL_TOTAL_CHARGE / 3 ) ) )
      ...
      if( client->ps.stats[ STAT_MISC ] > XAEL_TOTAL_CHARGE )
        client->ps.stats[ STAT_MISC ] = XAEL_TOTAL_CHARGE;

      if( client->ps.stats[ STAT_MISC ] > ( ammo * XAEL_TOTAL_CHARGE ) / 10 )
        client->ps.stats[ STAT_MISC ] = ammo * XAEL_TOTAL_CHARGE / 10;
    }
  ...
  while( client->time1000 >= 1000 )
  {
      ...
      level.surrenderTeam == PTE_HUMANS )
    {
      G_Damage( ent, NULL, NULL, NULL, NULL, 5, DAMAGE_NO_ARMOR, MOD_SUICIDE );
		...
		//If our HP is full, and people owe us something, clear it.
		if( ent->health >= client->ps.stats[ STAT_MAX_HEALTH ] && client->ps.stats[ STAT_PTEAM ] == (PTE_ALIENS || PTE_HUMANS) )
      {
    ...
    if( client->infected && !OnSameTeam( client->infector , ent ) )
    {
      ...
      if( client->infectionTime + 20000 > level.time )
      {
        ...
        for( i = 0; i < num; i++ )
        {
          ...
          {
            if( rand( ) % 100 > client->pers.aidresistance * 100 )
            {
...
//    G_Printf("serverTime >>>>>\n" );
  ...
  if( ( ( client->ps.stats[ STAT_STATE ] & SS_SLOWLOCKED ) || ( client->ps.stats[ STAT_STATE ] & SS_SLOWLOCKED2 ) ) &&
      client->lastSlowTime + ABUILDER_BLOB_TIME < level.time )
    {
      client->ps.stats[ STAT_STATE ] &= ~SS_SLOWLOCKED;
      client->ps.stats[ STAT_STATE ] &= ~SS_SLOWLOCKED2;
      client->blobs = 0;
    }
  ...
  if( !( ucmd->buttons & BUTTON_TALK ) && !( client->ps.pm_flags & PMF_RESPAWNED ) )
  {
    switch( client->ps.weapon )
    {
      ...
      case WP_ALEVEL0_UPG:
        if( client->ps.weaponTime <= 0 )
          pm.autoWeaponHit[ client->ps.weapon ] = CheckVenomAttack2( ent );
        break;
