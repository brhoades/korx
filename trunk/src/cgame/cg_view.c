...
#define LEVEL4_FEEDBACK  10.0f
  ...
  if( ps->weapon == WP_ALEVEL4 || ps->weapon == WP_ALEVEL4_UPG )
  {
...
#define LEVEL0_FEEDBACK  10.0f
  ...
  //provide some feedback for drill
  if( cg.predictedPlayerState.weapon == WP_ALEVEL0_UPG )
  {
    if( cg.predictedPlayerState.stats[ STAT_MISC ] > 0 )
    {
      float   fraction1, fraction2;
      ...
      fraction1 = (float)( cg.time - cg.weapon2Time ) / (float)LEVEL0_DRILL_CHARGE_TIME;
      ...
      VectorMA( origin, LEVEL0_FEEDBACK  * fraction2, forward, origin );
    }
  }
