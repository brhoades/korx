...
static void CG_TeamStatusDisplay( centity_t *cent )
{
  entityState_t   *es = &cent->currentState;
  vec3_t          origin;
  pTeam_t         pteam = cgs.clientinfo[ es->clientNum ].team;
  pClass_t        pclass = ( es->powerups >> 8 ) & 0xFF;
  int             health = cgs.clientinfo[ es->clientNum ].health;
  int             credits = cgs.clientinfo[ es->clientNum ].armor;
  float           healthScale = BG_FindHealthForClass( pclass );
  float           x, y;
  vec4_t          color;
  trace_t         tr;
  float           d;
  ...
  if( ( es->legsAnim & ~ANIM_TOGGLEBIT ) == LEGS_WALKCR ||
    ...
    BG_FindBBoxForClass( pclass,  mins, NULL, maxs, NULL, NULL );
  else
    BG_FindBBoxForClass( pclass,  mins, maxs, NULL, NULL, NULL );
  ...
  if( CG_WorldToScreen( origin, &x, &y ) )
  {
...

    {
      ...
      healthMax = BG_FindHealthForClass( pclass );
...
//blah :(
static int QDECL CG_SortDistance2( const void *a, const void *b )
{
  centity_t    *aent, *bent;
  float        adist, bdist;
  ...
  if( cg_drawTeamStatus.integer )
  {
    for( i = 0; i < cg.snap->numEntities; i++ )
    {
      ...
      if( es->eType == ET_PLAYER &&
          ( ( cent->currentState.powerups & 0x00FF ) == cg.predictedPlayerState.stats[ STAT_PTEAM ] ) )
