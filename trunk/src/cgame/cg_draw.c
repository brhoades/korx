...
static float CG_ChargeProgress( void )
{
  ...
  else if( cg.snap->ps.weapon == WP_ALEVEL4 || cg.snap->ps.weapon ==  WP_ALEVEL4_UPG)
  {
      min = 0;
      max = LEVEL4_CHARGE_TIME;
  }
  else if( cg.snap->ps.weapon == WP_LUCIFER_CANNON )
  {
    min = LCANNON_MIN_CHARGE;
    max = LCANNON_TOTAL_CHARGE;
  }
  else if( cg.snap->ps.weapon == WP_XAEL )
  {
    min = XAEL_MIN_CHARGE;
    max = XAEL_TOTAL_CHARGE;
  }
  if( max - min <= 0.f )
    return 0.f;
                                    ...
                                    qhandle_t shader )
{
  ...
  if( (cg.snap->ps.weapon == WP_LUCIFER_CANNON &&
      cg.snap->ps.stats[ STAT_MISC ] >= LCANNON_TOTAL_CHARGE-55 &&
      ( cg.time & 128 ) )
      ||
      (cg.snap->ps.weapon == WP_XAEL &&
      cg.snap->ps.stats[ STAT_MISC ] >= XAEL_TOTAL_CHARGE-55 &&
      ( cg.time & 128 ) ) )
  {
    color[ 0 ] = 1.f;
    color[ 1 ] = 0.f;
