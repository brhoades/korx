...
void CG_AddViewWeapon( playerState_t *ps )
{
  ...
  if( weapon == WP_XAEL && ps->stats[ STAT_MISC ] > 0 )
  {
    if( ps->stats[ STAT_MISC ] > ( XAEL_TOTAL_CHARGE - ( XAEL_TOTAL_CHARGE / 3 ) ) )
      trap_S_AddLoopingSound( ps->clientNum, ps->origin, vec3_origin, cgs.media.lCannonWarningSound );
  }
  ...
  if( weapon == WP_XAEL && ps->stats[ STAT_MISC ] > 0 )
  {
    float fraction = (float)ps->stats[ STAT_MISC ] / (float)XAEL_TOTAL_CHARGE;
